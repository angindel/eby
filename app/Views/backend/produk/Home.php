...
<script type="text/javascript">
		...
		var table = $('#tabel_serverside').DataTable({
			...
			'columnDefs': [
				...
				},
				{
					...
					data : 'harga_beli',
					render: function(data, type, row) {
						return new Intl.NumberFormat('id').format(parseInt(data));
					},
					width: '5%'
				},
				{
			...
			buttons: [
				{
					text: '<i class="fa-solid fa-plus fa-xl"></i> Produk',
					action: (e, dt, node, config) => {
						window.location.href = "<?= base_url("administrator/produk/tambah") ?>"
					},
        ...
        if(result.isConfirmed) {
          ...
          $.ajax({
            ...
            success: function(data){
              csrf.val(data.token);
              Swal.fire('Produk Berhasil Di Hapus','','success');
              table.ajax.reload();
            }
          });
        }
    ...
    $('#tabel_serverside tbody').on('click', '.edit-produk',function(){
    	...
    	window.location.href = window.location.href + `/edit/${e.id_produk}`;
    });
